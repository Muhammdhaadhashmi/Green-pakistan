...
public class UpdateProfileForCustomer extends AppCompatActivity {
    FirebaseAuth  mAuth ;
    ProgressDialog pd ;
    Uri resulturi;
    Uri newpicUri ;
 ...
 imageView.setOnClickListener(new View.OnClickListener() {
             ...
             public void onClick(View v) {
                  ...
                  InfoPhone.setEnabled(true) ; Infocity.setEnabled(true) ; InfoPassword.setEnabled(true) ; Infocnic.setEnabled(true) ; Infoage.setEnabled(true) ; InfoPrice.setEnabled(true) ; InfoStatus.setEnabled(false);
             }
         });
            ...
            public void onClick(View v) {
                if(resulturi==null) {
                    ...
                    if(Lattitude.isEmpty() || Longitude.isEmpty()){
                        Toast.makeText(UpdateProfileForCustomer.this, "Connecting", Toast.LENGTH_SHORT).show();
                        return;
                    }
             ...
             // update cons
                try {
                    UserModelClass userModelClass = new UserModelClass(
                            mAuth.getUid(), UserName, Emails, Password, Cnic, picUri, Phone, "Customer", Lattitude, Longitude, "true", "true", Rating);
                    UserFirebaseDatabasae.child(Keys).setValue(userModelClass);
                    Toast.makeText(UpdateProfileForCustomer.this, "Successfull", Toast.LENGTH_SHORT).show();



                }catch (Exception e){
                    Toast.makeText(UpdateProfileForCustomer.this, ""+e.toString(), Toast.LENGTH_SHORT).show();
                }
    ...
    private void PictureUpdate() {
        try {
            ...
            // update cons
                            UserModelClass userModelClass = new UserModelClass(
                                    mAuth.getUid() , UserName, Emails, Password, Cnic, uri.toString() , Phone,"Customer",Lattitude  , Longitude ,    "true"   , "false" , Rating   );
                            UserFirebaseDatabasae.child(Keys).setValue(userModelClass);
                            Toast.makeText(UpdateProfileForCustomer.this, "Successfull Pic", Toast.LENGTH_SHORT).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (null != picUri) {
                ...
                Picasso.with(getApplicationContext()).load(newpicUri);
            }
