...
public class UpdateProfileForCheff extends AppCompatActivity {
    FirebaseAuth  mAuth ;
    ProgressDialog pd ;
    Uri resulturi;
    Uri newpicUri ;
 ...
 imageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(resulturi==null) {
                    ...
                    if(Lattitude.isEmpty() || Longitude.isEmpty()){
                        Toast.makeText(UpdateProfileForCheff.this, "Connecting", Toast.LENGTH_SHORT).show();
                        return;
                    }
             ...
             // update cons
                try {
                    UserModelClass userModelClass = new UserModelClass(
                            mAuth.getUid(), UserName, Emails, Password, Cnic, picUri, Phone, "ShopKeeper", Lattitude, Longitude, "true", "true", Rating);
                    UserFirebaseDatabasae.child(Keys).setValue(userModelClass);
                    Toast.makeText(UpdateProfileForCheff.this, "Successfull", Toast.LENGTH_SHORT).show();



                }catch (Exception e){
                    Toast.makeText(UpdateProfileForCheff.this, ""+e.toString(), Toast.LENGTH_SHORT).show();
                }
    ...
    private void PictureUpdate() {
        try {
            ...
            // update cons
                            UserModelClass userModelClass = new UserModelClass(
                                    mAuth.getUid() , UserName, Emails, Password, Cnic, uri.toString() , Phone,"ShopKeeper",Lattitude  , Longitude ,    "true"   , "false" , Rating   );
                            UserFirebaseDatabasae.child(Keys).setValue(userModelClass);
                            Toast.makeText(UpdateProfileForCheff.this, "Successfull Pic", Toast.LENGTH_SHORT).show();
