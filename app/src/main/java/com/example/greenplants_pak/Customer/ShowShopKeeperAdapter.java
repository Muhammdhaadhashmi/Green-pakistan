...
public class ShowShopKeeperAdapter extends RecyclerView.Adapter<ShowShopKeeperAdapter.ViewHolder> implements Filterable {
    ...
    public ShowShopKeeperAdapter(Activity mContext, List<UserModelClass> list) {
 ...
 @NonNull
    @Override
    public ShowShopKeeperAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
       View v = LayoutInflater.from(mContext).inflate(R.layout.usersidelists , null);
        return new ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final ShowShopKeeperAdapter.ViewHolder holder, int position) {
        ...
        if(usersModelClass.getIsOnline().equals("true")){
           holder.circularImageView.setBackgroundResource(R.drawable.bt_uisquare_gree);
        }else{
            holder.circularImageView.setBackgroundResource(R.drawable.red_button_background);
        }
